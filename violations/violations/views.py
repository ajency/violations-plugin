...
def get_violations_data(filters={}):
	...
	if 'vio_id' in filters: ## -- If Violation ID is defined, then get that Violation data -- ##
		...
		action_filter_keys = ['who_ids', 'what_types']# , 'who_metas', 'what_metas']
		...
		if 'action_filters' in filters and any (k in filters['action_filters'] for k in action_filter_keys): ## -- If any action filters are implemented -- ##
			...
			# if 'what_metas' in filters['action_filters'] and filters['action_filters']['what_metas']: ## -- If the filter is defined for what_meta then search under 'what_meta' -- ##
	...
	response = {'data':json_data, 'total_count': query_data.count()} ## -- Get count to violations after applying all the filters -- ##
...
def comment_serializer(data=None): ## -- Method called for saving/updating `Comment` data in DB -- ##
	...
	if serializer.is_valid():
		...
		data = {"vio_id":serializer.validated_data['violation_id'], "who_id":serializer.validated_data['who_id'], "who_meta":serializer.validated_data['who_meta'], "what":"Violation Comment - action by " + eval(serializer.validated_data['who_meta'])['vio_role'] if 'vio_role' in eval(serializer.validated_data['who_meta']) else '-', "what_meta":{"comment_id": comment_obj.id}}
